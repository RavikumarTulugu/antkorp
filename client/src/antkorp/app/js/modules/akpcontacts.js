...
define(
		"akpcontacts",
		[ "jquery", "akpauth", "akpGroups", "jqueryui" ],
...
//deprecated
				...
				updateStatus:function(user){
					...
					if(user.status == 'donotdisturb'){
						...
						type = 'warning';
						user.status = 'idle';
					}else if(user.status == 'available' || user.status == 'online'){
						type = 'success';
						user.status = "available";
					}
					
					
					var pic = this.model.get("image_small") ||  "css/images/user32.png";
					var uimg = $("<div/>").append('<img src="'+ pic +'" alt="'+ this.model.get("first_name") +'" />').addClass("uimg pull-left");
					var ustat = $("<div/>").append('<span class="ustat_name">'+this.model.get("first_name")+'</span> is <span>'+user.status+'</span>').addClass("pull-left");
					var msg =$("<div/>").append(uimg).append(ustat); 
			...
			var userSelector = Backbone.View
					.extend({
						...
						settings : {},
						className : "userSelector Selector",
						events : {
							"click" : "getFocus",
						...
						initialize : function(options) {
							...
							this.collection.bind("remove", this._removeUser,									this);
						...
						render : function() {
							this.$el
									.append(
											'<div class="usersSelected Selected"><ul></ul></div><input type="text" class="userInput Input" placeholder="add" /><div class="userSuggestions Suggestions"><ul class="sugglist"></ul</div>')
						...
						addUser : function(model) {
							...
							$(" <span class='icon-cross removeUserTag removeTag'></span>")									.bind("click", {
										user : user.id
									}, function(e) {
						...
						_removeUser : function(model) {
							var user = model.toJSON();
							this._removeUserTag(user.id);
							this.trigger("removeUser",user);
						},
						_removeUserTag : function(id) {
							this.$(".usersSelected ul").children("li[data-uid=" + id + "]").remove();
						},
			...
			var GroupView = Backbone.View.extend({
				...
				deleteGroup : function(e) {
					if (this.group.isAdmin()) {
						...
						this.warnDelete(this.confirmDelete);
						
						//$(e.currentTarget).attr("disabled", "disabled");
					}
				},
				warnDelete:function(callback){
					var self = this;
					$("<div/>").append("Are you sure you want to delete group?").dialog({
				...
				confirmDelete:function(){
						...
						this.groups.deleteGroup(this.group.get("gid"),this.handleDeleteGroup);
				...
				addMember : function(model) {
					...
					var member = new App.Views.groupMember({
						model : model
					});
			...
			App.Views.groupMember = Backbone.View.extend({
						className : "member",
						events : {
						...
						render : function() {
							...
							var user = this.model.toJSON(),pic,template;
							
							
							if(this.viewType == "inline"){
								
								pic = user.image_small || "css/images/user32.png";
								...
								pic = user.image_large || "css/images/user96.jpg";
