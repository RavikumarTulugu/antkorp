...
/**
 * @fileOverview set of views and Application UI.
 * @name akpViews
 */

define("appViews",
		[ "jquery", "underscore", "backbone", "akputils", "akpauth",
				"akpGroups", "feedback", "plugins/jquery.steps.min", "plugins/jquery.knob","jquery.validate" ],
	...
	App.Views = App.Views || {};
			...
			var Workspace = Backbone.Router.extend({
				  ...
				  routes: {
					  '' : 'home',
					"groups": 'groups',// #groupSection
				    "groups/:gid(/:section)" : "app",    
				    "users/:uid(/:section)":"user",
				    "admin":"admin",
				    "login":"login",
				    "register":"register",
				    "wizard":"wizard",
				  ...
				  app:function(gid,section){
					  
						  groups.selectGroup(gid);
						  
						  if(section){
						  akp_ws.appView.showView("app",section);
					  }
				  },
				  groups:function(){
					  //console.log(arguments);
					  akp_ws.appView.showView("groups");
				  }
			...
			var appBaseModel = Backbone.Model.extend({
				...
				showView : function(view_name,opts) {
					this.trigger("viewChange", view_name,opts);
				},
				getView:function(){
					return this.get("mainView") ? this.get("mainView"): this.get("initialView");
				}
			});
			...
			var UserControls = Backbone.View.extend({
				...
				handleAddGroup : function(model) {
					if (model.isMember()) {
						...
						var link=$("<a/>").attr('href',"#/groups/"+modelData.gid).append(modelData.gname);
						
						var modelItem = $("<li/>").append(link)
						//.attr("data-gid",modelData.gid).append(modelData.gname)
								.addClass("groupItem");
					...
					.extend({
						...
						checkCookieStatus : function() {
							...
							if (result == null || result == ""){
								...
								//console	.log("user Cookie available. trying to login.");
						...
						sendLogin : function() {
							...
							if(args.length==3){
								if (!auth.loginuser(args[0], args[1],args[2],this.handleLoginResp))
									return false;
							}else if(args.length==1){
								if (!auth.loginuser(args[0], this.handleLoginResp))
									return false;
							}
						...
						sendLoginReq : function(e) {
							...
							this.sendLogin(obj.username, obj.password,obj.remember);
						},
						handleLoginResp : function(resp) {
							if (resp.status == "success" || resp.user) {
								...
								//if (this.store)// save on remember me
									setCookie("uht", resp.user.auth_token, 7);
						...
						serialize : function() {
							var store = this.$("#inputRememberMe").prop("checked");
			...
			var appGroupsView = Backbone.View.extend({
				...
				addGroup : function(model) {
					var group = new App.Views.GroupView({
						model : model,
				...
				select:function(gid){
					this.collection.selectGroup(gid);
				},
			...
			App.Views.GroupView = Backbone.View
					.extend({
						events : {
							//"click" : "selectGroup",
						...
						render : function() {
							var modeldata = this.model.toJSON(),template;
							modeldata.type = this.model.getType();
							modeldata.image_large = modeldata.image_large || "css/images/groupIcons/group128.png";
							modeldata.readOnly = this.settings.readOnly;
							

							if(this.settings.viewType=="inline"){
								template = $("#groupCardInline-template").tmpl([ modeldata ]);
								this.$el.addClass("medium");
							}
							else
							 template = $("#groupCard-template").tmpl([ modeldata ]);

							this.$el.attr("data-gid", modeldata.gid).append(template);
							...
							else if (!this.model.isMember(auth.loginuserid)) {
			...
			var resetPswdView = Backbone.View.extend({
				el:'.resetPswd_form',
				events:{
					'submit ' : 'sendResetPswd',
				},
				initialize:function(){
					_.bindAll(this,"handleResponse");
				...
				sendResetPswd:function(e){
					...
					var obj = {};
					var uname = $(e.target).find("#resetPswd_uname").val();
					var cpswd = $(e.target).find("#resetPswd_cpswd").val();
					var pswd = $(e.target).find("#resetPswd_pswd").val();
					auth.resetUserPswd(uname, pswd,this.handleResponse);
				...
				showError : function(form) {
					var msg = $("<div/>")
							...
							.append(									"Please fill all the fields correctly!");
					form.find(".form-info").empty().append(msg);
				},
				handleResponse : function(resp) {
					
					var form = this.$el,msg;
					if(typeof resp == 'String'){
						 msg = $("<div/>").addClass("alert alert-error").append(resp);
				
					}
					else if (resp.status == "success") {
						 msg = $("<div/>").addClass(	"alert alert-info").append("Password changed Successfully.");
						
					} else if(resp.error) {
						 msg = $("<div/>").addClass("alert alert-error").append(resp.error);
					}
					form.find(".form-info").empty().append(msg);
			...
			var adminView = Backbone.View
					.extend({
						...
						initialize : function() {
							...
							this.resetPswd = new resetPswdView;
							
						this.$(".addUser_form").validate({
						    debug:true,
						    rules: {
						        addUser_fname: {
						            "required":true,
						...
						showFSStatus:function(){
						    auth.getFSStatus(this.animateStats);
						},
						animateStats : function(resp) {
						    var fsusage= Math.round(resp.fsusage);
						    
						    this.$(".totalfs").html(utils.convBytes(parseInt(resp.folderLimit)));
						    this.$(".usedfs").html(utils.convBytes(parseInt(resp.folderUsage)));
							...
							this.$('.knob')	.each(
											function() {
												...
												var $this = $(this);
												var myVal = fsusage;//$this.attr("rel");
												// alert(myVal);
												$this.knob({
													'draw' : function() {
														$(this.i).val(
			...
			var navView = Backbone.View
					.extend({
						...
						initialize : function(opts) {
							...
							this.baseModel.bind("viewChange",this.handleViewChange, this);
							
							

						},
						handleViewChange : function(view,opts) {
							if (view == "app") {
								
								view = opts ? opts : this.model.getView();
							}
						...
						_changeView:function(tab){
							this.$("li").removeClass("active");
							this.$("li[data-tabId=" + tab + "]").addClass("active");

							$(".content").children().hide().end().children('#' + tab).show();
						},
						changeState : function(tab, sub) {
							...
							this._changeView(tab);
							var cwg=this.baseModel.get("currentGroup");
							
							/*if(cwg)
								app_router.navigate("#/groups/"+cwg+'/'+tab, {trigger:false,replace:true,});
								*/
							...
							if (typeof this.settings.onViewChange[tab] === 'function')
			...
			var appFlow = Backbone.View
					.extend({
						...
						showApp : function(group) {
							...
							this.model.set({"currentGroup":group.gid});
						...
						showView : function(viewname,args) {
							...
							if (viewname == "3stepwizard") {
								...
								this.model.showView("user");
							}else if(viewname=="groups"||viewname=="app"){
								this.model.showView(viewname,args);
							}
						}
					});
