...
(function($)
{
		...
		else if(typeof option == 'string')
		{
			...
			var elements = this.each(function()
			{
				...
				if(data)
				{
					...
					else if(option == 'render'){ data.render(); }
					else if(option == "members"){data.getMembers();}
					...
					else if(option == 'drawData' && settings !== undefined){data.drawData(settings);}
					else if(option == 'mode' && settings !== undefined){data.changeMode(settings);}
	...
	$.fn.wPaint.defaultSettings = {
		...
		menu 				 : ['undo','clear','rectangle','ellipse','line','pencil','text','eraser','fillColor','lineWidth','strokeColor',"share"], // menu items - appear in order they are set
	...
	{	
		...
		callFunc: function(e, _self, event)
		{
			...
			if(_self.settings['draw' + event]) _self.settings['draw' + event].apply(_self, [e, mode,event]);
		...
		setGroup:function(){
			var _self=this;
			var selector=$('<div class="modal" style="top:50%; left:50%;" ><input type="text" id="userSelector" /><button id="userSelectDone">Done</button></div>').appendTo('body');
			selector.find("#userSelectDone").click(function(){
				var member=selector.find("#userSelector").val();
				var group=_self.settings.group;
				group.push(member);
				selector.remove();
			})
		},
	...
	MainMenu.prototype = 
	{
		generate: function(canvas, textMenu)
		{
			...
			$.each($canvas.settings.menu, function(i, item)
			{
				switch(item)
				{
					...
					case 'share':menuContent.append($('<div class="_wPaint_btn _wpaint_share" title="Share Board">Share</div>').click(function(e){$canvas.setGroup();})); break;
				}
			});
			...
			//FIXME: when you need handle remove comments
			/*.draggable({
				handle: menuHandle, 
		...
		set_mode: function(_self, $canvas, mode)
		{
			...
			$canvas.settings["onChangeMode"].apply($canvas,[mode]);
	...
	TextMenu.prototype = 
	{
		generate: function(canvas, mainMenu)
		{
			...
			//FIXME: Remove comments to enable handle and draggabe
			/*.draggable({
				snap: '._wPaint_menu', 
				handle: menuHandle,
				stop: function(){
