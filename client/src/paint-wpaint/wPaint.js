...
(function($)
{
		...
		else if(typeof option == 'string')
		{
			...
			var elements = this.each(function()
			{
				...
				if(data)
				{
					...
					else if(option == "draw" && settings !== undefined){ data.paint(settings.e,settings.event,settings.args)}
	...
	{	
		...
		callFunc: function(e, _self, event)
		{
			...
			var temp=_self.settings;

			if(_self.settings['draw' + event]) _self.settings['draw' + event].apply(_self, [e, mode,event,temp]);

			
		},
		paint:function(e, event,args){
			
			var _self=this;
           $e = jQuery.extend(true, {}, e);
           
           delete args.drawUp;
           delete args.drawDown;
           delete args.drawMove;
           
           $.extend(true,_self.settings,args);
			...
			if(mode !== 'Text' && event === 'Up') { this.addUndo(); }
		},
